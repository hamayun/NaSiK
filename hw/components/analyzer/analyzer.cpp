...
Analyzer::Analyzer(char * elf_file, uintptr_t app_base_addr, annotation_buffer_t * buffers,
  ...
  : _buffers(buffers),
    ...
    _analyze(analyze)
    {
      ...
      DOUT_FCT << " Application " << elf_file << " at " << std::hex << _appli_base_addr << std::endl;
      load_symtab(elf_file);
      sem_init(&_sem, 0, 1); 
    ...
    inline void Analyzer::push_context(basicblock_t *bb)
    {
      ...
      if(_current_thread_slot->prev_bb != NULL)
      {
        ...
        if( calls_it == _current_thread_slot->prev_bb->calls->end() )
        {
          call_ptr = new call_t;
          *call_ptr = INIT_CALL;
          // Find symbol of the function containing the mbb_annotation call.
