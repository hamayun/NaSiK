...
#define DELIM           " \t-="
#define NATIVE_TOKEN     "native"
#define NATIVE_HELP      "help"
#define NATIVE_DEBUG     "debug"
#define NATIVE_OPTION    "option"
...
#define STRCMP_TOKEN(s1,s2)    strncmp(s1,s2,strlen(s2))
	...
	int parseCommandLine(char *argv[])
	{
		...
		while(token != NULL)
		{
			...
			if(STRCMP_TOKEN(token,NATIVE_TOKEN) == 0)
			{
				token = next_token( argv, DELIM);
				if(STRCMP_TOKEN(token,NATIVE_HELP) == 0)
				{
				}
				if(STRCMP_TOKEN(token,NATIVE_DEBUG) == 0)
				{
				...
				if(STRCMP_TOKEN(token,NATIVE_OPTION) == 0)
				{
					module = next_token( argv, " \t=");
					while(module != NULL)
					{	
