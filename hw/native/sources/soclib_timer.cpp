...
void soclib_timer::end_of_elaboration()
{
  ...
  char        *token;
	...
	_segments.push_back(mapping::init(name(), TIMER_SPAN * sizeof(uint32_t) * _nb_timers->value));
  ...
  // SOCLIB_TIMER_DEVICES = { timer_0.addr, timer_1.addr  ... }
  //symbol_value = new symbol<uint32_t>("SOCLIB_TIMER_DEVICES");
  
  symbol_value = new symbol<uint32_t>("PLATFORM_TIMER_INTN");
  symbol_value->push_back(atoi(_irq_config->value));
  _symbols.push_back(symbol_value);

  symbol_value = new symbol<uint32_t>("PLATFORM_TIMER_BASE");
