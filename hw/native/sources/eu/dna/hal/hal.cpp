...
extern "C" {
  ...
  void __dnaos_hal_write_uint64(uint64_t *addr, uint64_t value)
  {
    SYNCHRONIZE();
    current_eu->write(addr,value);
  }

  void __dnaos_hal_write_sfloat(float *addr, float value)
  {
    SYNCHRONIZE();
    current_eu->write(addr,value);
  }

  void __dnaos_hal_write_dfloat(double *addr, double value)
  {
    SYNCHRONIZE();
    current_eu->write(addr,value);
  }
  ...
  uint64_t __dnaos_hal_read_uint64(uint64_t *addr)
  {
    SYNCHRONIZE();
    return(current_eu->read(addr));
  }

  float __dnaos_hal_read_sfloat(float *addr)
  {
    SYNCHRONIZE();
    return(current_eu->read(addr));
  }

  double __dnaos_hal_read_dfloat(double *addr)
  {
    SYNCHRONIZE();
    return(current_eu->read(addr));
  }

  void __dnaos_hal_vector_write_dfloat(double *to,double *from, unsigned long int len)
  {
    double val;

    for(uint32_t i = 0; i < len; i++)
    {
      val = __dnaos_hal_read_dfloat(from + i);
      __dnaos_hal_write_dfloat(to + i, val);
    }
  }

  void __dnaos_hal_vector_write_sfloat(float *to,float *from, unsigned long int len)
  {
    float val;

    for(uint32_t i = 0; i < len; i++)
    {
      val = __dnaos_hal_read_sfloat(from + i);
      __dnaos_hal_write_sfloat(to + i, val);
    }
  }

  void __dnaos_hal_vector_write_uint64(uint64_t *to, uint64_t *from, unsigned long int len)
  {
    uint64_t val;
 
    for(uint32_t i = 0; i < len; i++)
    {
      val = __dnaos_hal_read_uint64(from + i);
      __dnaos_hal_write_uint64(to + i, val);
    }
  }

  void __dnaos_hal_vector_write_uint32(uint32_t *to, uint32_t *from, unsigned long int len)
  {
    uint32_t val;

    for(uint32_t i = 0; i < len; i++)
    {
      val = __dnaos_hal_read_uint32(from + i);
      __dnaos_hal_write_uint32(to + i, val);
    }
  }

  void __dnaos_hal_vector_write_uint16(uint16_t *to, uint16_t *from, unsigned long int len)
  {
    uint16_t val;

    for(uint32_t i = 0; i < len; i++)
    {
      val = __dnaos_hal_read_uint16(from + i);
      __dnaos_hal_write_uint16(to + i, val);
    }
  }

  void __dnaos_hal_vector_write_uint8 (uint8_t *to, uint8_t *from, unsigned long int len)
  {
    uint8_t val;

    for(uint32_t i = 0; i < len; i++)
    {
      val = __dnaos_hal_read_uint8(from + i);
      __dnaos_hal_write_uint8(to + i, val);
    }
  }
