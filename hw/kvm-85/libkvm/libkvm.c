...
again:
	...
	if (r){
            fprintf(stderr, "pre_kvm_run: ret = %d\n", r);
	    return r;
        }
	...
	if (1) {
		...
		case KVM_EXIT_IO:
			r = handle_io(kvm, run, vcpu);
                        if(r) fprintf(stderr, "kvm_run: handle_io error ret = %d\n", r);
			break;
		case KVM_EXIT_DEBUG:
			r = handle_debug(kvm, vcpu, env);
                        if(r) fprintf(stderr, "kvm_run: handle_debug error ret = %d\n", r);
			break;
		case KVM_EXIT_MMIO:
			r = handle_mmio(kvm, run);
                        if(r) fprintf(stderr, "kvm_run: handle_mmio error ret = %d\n", r);
			break;
		case KVM_EXIT_HLT:
			r = handle_halt(kvm, vcpu);
                        if(r) fprintf(stderr, "kvm_run: handle_halt error ret = %d\n", r);
			break;
		case KVM_EXIT_IRQ_WINDOW_OPEN:
			break;
		case KVM_EXIT_SHUTDOWN:
			r = handle_shutdown(kvm, env);
                        if(r) fprintf(stderr, "kvm_run: handle_shutdown error ret = %d\n", r);
			break;
#if defined(__s390__)
