...
int32_t ProduceMemWriteBack(C62x_MWB_Queue_t * mwb_queue, C62xMWB_Size_t size, uint32_t addr, uint32_t value)
{
...
int32_t AddMemWriteBack(C62x_Proc_State_t * proc_state, C62xMWB_Size_t size, uint32_t addr, uint32_t value, uint8_t delay)
{
    uint32_t           mwb_queue_id = (proc_state->m_curr_cpu_cycle + delay + 1) % (C62X_MAX_DELAY_SLOTS + 1);
    C62x_MWB_Queue_t * mwb_queue    = & proc_state->m_mwback_q[mwb_queue_id];

    if(ProduceMemWriteBack(mwb_queue, size, addr, value))
    {
        return (-1);
    }
    return (0);
                        ...
                        uint32_t constant, uint16_t idx_rbh, uint16_t idx_rbl, uint16_t idx_rdh, uint16_t idx_rdl, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
        ...
        TRACE_PRINT("%08x\tADD       0x%x,%s:%s,%s:%s\n",
                GetPC(proc_state), constant, REG(idx_rbh), REG(idx_rbl), REG(idx_rdh), REG(idx_rdl));
    }
    return OK;
}
                        ...
                        int32_t constant,  uint16_t idx_rbh, uint16_t idx_rbl, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
        ...
        TRACE_PRINT("%08x\tCMPEQ     0x%x,%s:%s,%s\n",
                GetPC(proc_state), constant, REG(idx_rbh), REG(idx_rbl), REG(idx_rd));
    }
    return OK;
}
...
ReturnStatus_t
C62xSHL_SR32_UR32_SR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                        uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
        int32_t ra     = (int32_t) proc_state->m_register[idx_ra];
        int32_t shift  = (int32_t) proc_state->m_register[idx_rb] & 0x3F;
...
ReturnStatus_t
C62xSHL_SR40_UR32_SR40(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                       uint16_t idx_rah, uint16_t idx_ral, uint16_t idx_rb, uint16_t idx_rdh, uint16_t idx_rdl, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
        uint32_t rah   = (uint32_t) proc_state->m_register[idx_rah];
        uint32_t ral   = (uint32_t) proc_state->m_register[idx_ral];
        int32_t shift  = (int32_t)  proc_state->m_register[idx_rb] & 0x3F;
        ...
        TRACE_PRINT("%08x\tSHL       %s:%s,%s,%s:%s\n",
                GetPC(proc_state), REG(idx_rah), REG(idx_ral), REG(idx_rb), REG(idx_rdh), REG(idx_rdl));
    }
    return OK;
}

ReturnStatus_t
C62xSHL_UR32_UR32_UR40(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                       uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rdh, uint16_t idx_rdl, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
        uint64_t ra    = (uint64_t) proc_state->m_register[idx_ra];
        int32_t shift  = (int32_t)  proc_state->m_register[idx_rb] & 0x3F;
                      ...
                      uint16_t idx_ra, uint32_t constant, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
        ...
        TRACE_PRINT("%08x\tSHL       %s,0x%x,%s\n",
                GetPC(proc_state), REG(idx_ra), shift, REG(idx_rd));
    }
    return OK;
}

ReturnStatus_t
C62xSHL_SR40_UC5_SR40(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                       uint16_t idx_rah, uint16_t idx_ral, uint32_t constant, uint16_t idx_rdh, uint16_t idx_rdl, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
        ...
        TRACE_PRINT("%08x\tSHL       %s:%s,0x%x,%s:%s\n",
                GetPC(proc_state), REG(idx_rah), REG(idx_ral), shift, REG(idx_rdh), REG(idx_rdl));
    }
    return OK;
}

ReturnStatus_t
C62xSHL_UR32_UC5_SR40(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                       uint16_t idx_ra, uint32_t constant, uint16_t idx_rdh, uint16_t idx_rdl, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
        ...
        TRACE_PRINT("%08x\tSHL       %s,0x%x,%s:%s\n",
                GetPC(proc_state), REG(idx_ra), shift, REG(idx_rdh), REG(idx_rdl));
    }
    return OK;
}
 ...
 */
ReturnStatus_t
C62xSHR_SR32_UR32_SR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                        uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
...
ReturnStatus_t
C62xSHR_SR40_UR32_SR40(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                       uint16_t idx_rah, uint16_t idx_ral, uint16_t idx_rb, uint16_t idx_rdh, uint16_t idx_rdl, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
...
ReturnStatus_t
C62xSHR_SR32_UC5_SR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                      uint16_t idx_ra, uint32_t constant, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
...
ReturnStatus_t
C62xSHR_SR40_UC5_SR40(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                       uint16_t idx_rah, uint16_t idx_ral, uint32_t constant, uint16_t idx_rdh, uint16_t idx_rdl, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
 ...
 */
ReturnStatus_t
C62xSHRU_UR32_UR32_UR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                        uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
        uint32_t ra    = (uint32_t) proc_state->m_register[idx_ra];
        uint32_t shift = (uint32_t) proc_state->m_register[idx_rb] & 0x3F;
...
ReturnStatus_t
C62xSHRU_UR40_UR32_UR40(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                       uint16_t idx_rah, uint16_t idx_ral, uint16_t idx_rb, uint16_t idx_rdh, uint16_t idx_rdl, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
...
ReturnStatus_t
C62xSHRU_UR40_UC5_UR40(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                       uint16_t idx_rah, uint16_t idx_ral, uint32_t constant, uint16_t idx_rdh, uint16_t idx_rdl, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
 ...
 */
ReturnStatus_t
C62xSMPY_SR16_SR16_SR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                        uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
...
ReturnStatus_t
C62xSMPYH_SR16_SR16_SR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                        uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
...
ReturnStatus_t
C62xSMPYHL_SR16_SR16_SR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                          uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
...
ReturnStatus_t
C62xSMPYLH_SR16_SR16_SR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                          uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
 ...
 */
ReturnStatus_t
C62xSSHL_SR32_UR32_SR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                        uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
        int32_t ra     = (int32_t) proc_state->m_register[idx_ra];
        int32_t shift  = (int32_t) proc_state->m_register[idx_rb] & 0x1F; // 0x3F for C64x !!!
        int32_t sflag  = 0;
...
ReturnStatus_t
C62xSSHL_SR32_UC5_SR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                        uint16_t idx_ra, uint32_t constant, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
...
ReturnStatus_t
C62xSSUB_SR32_SR32_SR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                        uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
...
ReturnStatus_t
C62xSSUB_SC5_SR32_SR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                       uint32_t constant, uint16_t idx_rb, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
...
ReturnStatus_t
C62xSSUB_SC5_SR40_SR40(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                       uint32_t constant, uint16_t idx_rbh, uint16_t idx_rbl, uint16_t idx_rdh, uint16_t idx_rdl, uint8_t delay)
{
    ASSERT(1, "Test Again");
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
...
/*  For SSUB
    ...
    proc_state->m_register[idx_rbh] = 0x7F;
    proc_state->m_register[idx_rbl] = 0xFFFFFFFF;
 */
...
ReturnStatus_t
C62xSTB_UC15_UR32_UR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                       uint32_t constant, uint16_t idx_rb, uint16_t idx_rd, uint8_t mode, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
...
ReturnStatus_t
C62xSTH_UC15_UR32_UR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                       uint32_t constant, uint16_t idx_rb, uint16_t idx_rd, uint8_t mode, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
...
ReturnStatus_t
C62xSTW_UC15_UR32_UR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                       uint32_t constant, uint16_t idx_rb, uint16_t idx_rd, uint8_t mode, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
...
ReturnStatus_t
C62xSUB_SR32_SR32_SR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                        uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
...
ReturnStatus_t
C62xSUB_SR32_SR32_SR40(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                        uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rdh, uint16_t idx_rdl, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
...
ReturnStatus_t
C62xSUB_SC5_SR32_SR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                      uint32_t constant, uint16_t idx_rb, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
...
ReturnStatus_t
C62xSUB_SC5_SR40_SR40(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                      uint32_t constant, uint16_t idx_rbh, uint16_t idx_rbl, uint16_t idx_rdh, uint16_t idx_rdl, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
...
ReturnStatus_t
C62xSUB_SR32_SC5_SR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                      uint16_t idx_ra, uint32_t constant, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
        ...
        TRACE_PRINT("%08x\tSUB       %s,0x%x,%s\n",
                GetPC(proc_state), REG(idx_ra), constant, REG(idx_rd));
    }
    return OK;
}

ReturnStatus_t
C62xSUB_SR32_UC5_SR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                      uint16_t idx_ra, uint32_t constant, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
...
ReturnStatus_t
C62xSUB_SR40_SC5_SR40(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                      uint16_t idx_rah, uint16_t idx_ral, uint32_t constant, uint16_t idx_rdh, uint16_t idx_rdl, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
        ...
        TRACE_PRINT("%08x\tSUB       %s:%s,0x%x,%s:%s\n",
                GetPC(proc_state), REG(idx_rah), REG(idx_ral), constant, REG(idx_rdh), REG(idx_rdl));
    }
    return OK;
}
...
ReturnStatus_t
C62xSUBAB_SR32_SR32_SR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                        uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
...
ReturnStatus_t
C62xSUBAB_SR32_UC5_SR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                        uint16_t idx_ra, uint32_t constant, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
...
ReturnStatus_t
C62xSUBAH_SR32_SR32_SR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                        uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
...
ReturnStatus_t
C62xSUBAH_SR32_UC5_SR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                        uint16_t idx_ra, uint32_t constant, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
...
ReturnStatus_t
C62xSUBAW_SR32_SR32_SR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                         uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
...
ReturnStatus_t
C62xSUBAW_SR32_UC5_SR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                        uint16_t idx_ra, uint32_t constant, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
 ...
 */
ReturnStatus_t
C62xSUBC_UR32_UR32_UR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                        uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
...
ReturnStatus_t
C62xSUBU_UR32_UR32_UR40(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                       uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rdh, uint16_t idx_rdl, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
 ...
 */
ReturnStatus_t
C62xSUB2_UR32_UR32_UR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                        uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
        uint32_t ra = (int32_t) proc_state->m_register[idx_ra];
        uint32_t rb = (int32_t) proc_state->m_register[idx_rb];
...
ReturnStatus_t
C62xXOR_SC5_UR32_UR32(C62x_Proc_State_t * proc_state, uint8_t is_cond, uint8_t be_zero, uint16_t idx_rc,
                      uint32_t constant, uint16_t idx_rb, uint16_t idx_rd, uint8_t delay)
{
    if(ExecuteDecision(proc_state, is_cond, be_zero, idx_rc))
    {
