...
int32_t EnQ_Delay_Result(C62x_DSPState_t * p_state, C62x_Result_t * result, uint8_t delay_slots)
{
    ...
    if(result->m_type & C62X_REGISTER)
    {
        ...
        tail_node->m_value  = result->m_value[RESULT_LREG_IDX];
        p_state->m_delay_q[queue_id].m_tail_node = tail_node->m_next_node;
    ...
    if(result->m_type & C62X_MULTIREG)
    {
        ...
        tail_node->m_reg_id = result->m_reg_id[RESULT_HREG_IDX];
        tail_node->m_value  = result->m_value[RESULT_HREG_IDX];
...
int32_t Update_Immediate(C62x_DSPState_t * p_state, C62x_Result_t * result)
{
    ...
    if(result->m_type & C62X_MULTIREG)
    {
        ...
        p_state->m_reg[result->m_reg_id[RESULT_LREG_IDX]] = result->m_value[RESULT_LREG_IDX];
        p_state->m_reg[result->m_reg_id[RESULT_HREG_IDX]] = result->m_value[RESULT_HREG_IDX];
        result->m_type &= ~C62X_MULTIREG;
    }
                  ...
                  uint16_t idx_rah, uint16_t idx_ral, uint16_t idx_rdh, uint16_t idx_rdl, uint8_t delay, C62x_Result_t * result)
{
    if(Check_Predicate(p_state, is_cond, be_zero, idx_rc))
    {
        int32_t rah = p_state->m_reg[idx_rah];
        ...
        SAVE_MREGS_RESULT(result, idx_rdh, rdh, idx_rdl, rdl);
                        ...
                        uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rd, uint8_t delay, C62x_Result_t * result)
{
    if(Check_Predicate(p_state, is_cond, be_zero, idx_rc))
    {
        int32_t ra   = p_state->m_reg[idx_ra];
        int32_t rb   = p_state->m_reg[idx_rb];
        uint32_t amr = p_state->m_reg[REG_AMR_INDEX];
                        ...
                        uint32_t constant, uint16_t idx_rd, uint8_t delay, C62x_Result_t * result)
{
    if(Check_Predicate(p_state, is_cond, be_zero, idx_rc))
    {
        ...
        int32_t rd  = ra + p_state->m_reg[idx_rd];
                        ...
                        uint32_t constant, uint8_t delay, C62x_Result_t * result)
{
    if(Check_Predicate(p_state, is_cond, be_zero, idx_rc))
    {
        ...
        SAVE_REG_RESULT(result, REG_PC_INDEX, rd);
...
int8_t * FindMemoryAddress(C62x_DSPState_t * p_state, uint32_t base_reg, uint16_t base_idx,
                           uint32_t offset, uint8_t mode, C62xAlignment_t shift, C62x_Result_t * result)
{
    ...
    switch(mode)
    {
        ...
        case REG_OFFSET_PRE_DECR:        // *--R[offsetR]
            ...
            ptr = (int8_t *) base_reg;
            SAVE_SREG_UPDATE(result, base_idx, base_reg);
            break;
                       ...
                       uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rd, uint8_t mode, uint8_t delay, C62x_Result_t * result)
{
    if(Check_Predicate(p_state, is_cond, be_zero, idx_rc))
    {
        ...
        int8_t * ptr  = FindMemoryAddress(p_state, rb, idx_rb, ra, mode, BYTE_ALIGN, result);
                      ...
                      uint32_t constant, uint16_t idx_rb, uint16_t idx_rd, uint8_t mode, uint8_t delay, C62x_Result_t * result)
{
    if(Check_Predicate(p_state, is_cond, be_zero, idx_rc))
    {
        ...
        int8_t * ptr  = FindMemoryAddress(p_state, rb, idx_rb, constant, mode, BYTE_ALIGN, result);
                       ...
                       uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rd, uint8_t mode, uint8_t delay, C62x_Result_t * result)
{
    if(Check_Predicate(p_state, is_cond, be_zero, idx_rc))
    {
        ...
        int8_t * ptr  = FindMemoryAddress(p_state, rb, idx_rb, ra, mode, HWORD_ALIGN, result);
                        ...
                        uint32_t constant, uint16_t idx_rb, uint16_t idx_rd, uint8_t mode, uint8_t delay, C62x_Result_t * result)
{
    if(Check_Predicate(p_state, is_cond, be_zero, idx_rc))
    {
        ...
        int8_t * ptr  = FindMemoryAddress(p_state, rb, idx_rb, constant, mode, HWORD_ALIGN, result);
                       ...
                       uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rd, uint8_t mode, uint8_t delay, C62x_Result_t * result)
{
    if(Check_Predicate(p_state, is_cond, be_zero, idx_rc))
    {
        ...
        int8_t * ptr  = FindMemoryAddress(p_state, rb, idx_rb, ra, mode, WORD_ALIGN, result);
                      ...
                      uint32_t constant, uint16_t idx_rb, uint16_t idx_rd, uint8_t mode, uint8_t delay, C62x_Result_t * result)
{
    if(Check_Predicate(p_state, is_cond, be_zero, idx_rc))
    {
        ...
        int8_t * ptr  = FindMemoryAddress(p_state, rb, idx_rb, constant, mode, WORD_ALIGN, result);
                 ...
                 uint16_t idx_rah, uint16_t idx_ral, uint16_t idx_rdh, uint16_t idx_rdl, uint8_t delay, C62x_Result_t * result)
{
    if(Check_Predicate(p_state, is_cond, be_zero, idx_rc))
    {
        int32_t rdh = p_state->m_reg[idx_rah];
        int32_t rdl = p_state->m_reg[idx_ral];
                        ...
                        uint16_t idx_ra, uint16_t idx_rd, uint8_t delay, C62x_Result_t * result)
{
    if(Check_Predicate(p_state, is_cond, be_zero, idx_rc))
    {
        int32_t rd = p_state->m_reg[idx_ra];
                   ...
                   uint32_t constant, uint16_t idx_rd, uint8_t delay, C62x_Result_t * result)
{
    if(Check_Predicate(p_state, is_cond, be_zero, idx_rc))
    {
        int32_t rd = p_state->m_reg[idx_rd];
                        ...
                        uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rd, uint8_t delay, C62x_Result_t * result)
{
    if(Check_Predicate(p_state, is_cond, be_zero, idx_rc))
    {
        ...
        if(sflag)
        {
            uint32_t csr = p_state->m_reg[REG_CSR_INDEX];
                       ...
                       uint16_t idx_rah, uint16_t idx_ral, uint16_t idx_rb, uint16_t idx_rdh, uint16_t idx_rdl, uint8_t delay, C62x_Result_t * result)
{
    if(Check_Predicate(p_state, is_cond, be_zero, idx_rc))
    {
        uint32_t rah   = p_state->m_reg[idx_rah];
                        ...
                        uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rd, uint8_t delay, C62x_Result_t * result)
{
    ...
    {
        int32_t ra     = p_state->m_reg[idx_ra];
        int32_t shift  = p_state->m_reg[idx_rb] & 0x1F; // 0x3F for C64x !!!
        int32_t sflag  = 0;
                       ...
                       uint16_t idx_ra, uint16_t idx_rb, uint16_t idx_rd, uint8_t mode, uint8_t delay, C62x_Result_t * result)
{
    if(Check_Predicate(p_state, is_cond, be_zero, idx_rc))
    {
        uint32_t ra   = p_state->m_reg[idx_ra];
        uint32_t rb   = p_state->m_reg[idx_rb];
        uint32_t src  = p_state->m_reg[idx_rd];
        uint32_t amr  = p_state->m_reg[REG_AMR_INDEX];
              ...
              uint16_t idx_rd, uint8_t delay, C62x_Result_t * result)
{
    if(Check_Predicate(p_state, is_cond, be_zero, idx_rc))
    {
        SAVE_REG_RESULT(result, idx_rd, 0);
              ...
              uint16_t idx_rdh, uint16_t idx_rdl, uint8_t delay, C62x_Result_t * result)
{
    if(Check_Predicate(p_state, is_cond, be_zero, idx_rc))
    {
        SAVE_MREGS_RESULT(result, idx_rdh, 0x0, idx_rdl, 0x0);
