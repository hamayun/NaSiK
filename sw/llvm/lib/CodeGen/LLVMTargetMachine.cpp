...
bool LLVMTargetMachine::CompileModuleForTargetFlag = false;
...
bool LLVMTargetMachine::addPassesToEmitFile(PassManagerBase &PM,
  ...
  DEBUG (dbgs() << "CompileModuleForTargetFlag = " << CompileModuleForTargetFlag << "\n");
  if((CompileModuleForTargetFlag == false) && (AnnotationArch != ""))
  {
    assert( mod != NULL && "A module is needed for annotation !!!");

    DEBUG (dbgs() << __func__ << ": compile module for target " << AnnotationArch << "\n");
    compileModuleForTarget(mod, OptLevel, DisableVerify);
  }
  ...
  {
    DEBUG (dbgs() << __func__ << ": add target annotation pass ...\n");
    if (targetSupportAnnotation()) {
      PM.add(createMachineTargetAnnotationPass(*this));
    } else {
      std::cerr << "Warning: target annotation not supported!!!\n";
    }
  }
...
int LLVMTargetMachine::addCFGPrinter(PassManagerBase &PM, std::string checkpoint)
{
    static int callcount = 0;
    std::ostringstream scallcount;

    if(CompileModuleForTargetFlag)
    {
        scallcount << callcount++;
        std::string str = "." + scallcount.str() + "_" + checkpoint;

        //PM.add(createCFGOnlyPrinterPass(str));
        //PM.add(createCFGPrinterPass(checkpoint+"_detailed"));
    }
    return 0;
}
...
bool LLVMTargetMachine::addCommonCodeGenPasses(PassManagerBase &PM,
  ...
  if(addCFGPrinter(PM, "UnreachableBlockElim"))
    return true;
  ...
  if(addCFGPrinter(PM, "StackSlotAllocation"))
    return true;
  ...
  if(addCFGPrinter(PM, "PreRegAlloc"))
    return true;
  ...
  if(addCFGPrinter(PM, "PostRegAlloc"))
    return true;
  ...
  if(addCFGPrinter(PM, "PrologEpilog"))
    return true;
  ...
  if(addCFGPrinter(PM, "GCMachineCodeAnalysis"))
    return true;
  ...
  if(addCFGPrinter(PM, "PreEmitPass"))
    return true;
                                               ...
                                               bool DisableVerify)
{
    ...
    // Here we select the target machine for annotation purposes.
    // 'AnnotationArch' should be a valid architecture here.
    const Target *AnnotationTarget = 0;

    for (TargetRegistry::iterator it = TargetRegistry::begin(),
         ie = TargetRegistry::end(); it != ie; ++it)
    {
        if (AnnotationArch == it->getName())
        {
            AnnotationTarget = &*it;
            break;
        }
    ...
    assert(AnnotationTarget != NULL && "Target annotated architecture not found !!!");
    ...
    std::auto_ptr<TargetMachine> target(AnnotationTarget->createTargetMachine(TheTriple.getTriple(), FeaturesStr));
