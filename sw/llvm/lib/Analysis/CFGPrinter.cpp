...
namespace {
  struct CFGOnlyPrinter : public FunctionPass {
    static char ID; // Pass identification, replacement for typeid
    std::string strfname;
    CFGOnlyPrinter(std::string str = "") : FunctionPass(ID), strfname(str) {}
    virtual bool runOnFunction(Function &F) {
        std::string Filename;
        if (strfname != "")
            Filename = "cfg." + F.getNameStr() + strfname + ".dot";
        else
            Filename = "cfg." + F.getNameStr() + ".dot";
...
FunctionPass *llvm::createCFGOnlyPrinterPass (std::string str) {
  return new CFGOnlyPrinter(str);
}
