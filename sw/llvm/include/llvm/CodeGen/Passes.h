...
namespace llvm {
  ...
  /// createMachineCheckCFGPass - This pass performs checking on machine
  /// CFG and LLVM CFG
  // FunctionPass *createMachineCheckCFGPass(const char* previousPass);

  /// createMachineTargetAnnotationPass - This pass insert annotations
  /// for a target arch in the LLVM level CFG. If generated, the BC file
  /// can be recompiled using Host Annotation Pass to be analyzed.
  FunctionPass *createMachineTargetAnnotationPass(TargetMachine &T);

  /// createCFGPrinterPass - This pass create LLVM and Machine Basic Block
  /// CFG dot files.
  FunctionPass *createCFGPrinterPass(const std::string &name);
