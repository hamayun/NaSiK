...
enum
{
	...
	/* options for KVM */
	CMDLINE_OPTION_kvm_bootstrap,
	CMDLINE_OPTION_kvm_software,
};
...
const cmdline_option cmdline_options[] = 
{
	...
	{"kvm_bootstrap", HAS_ARG, CMDLINE_OPTION_kvm_bootstrap},
	{"kvm_software", HAS_ARG, CMDLINE_OPTION_kvm_software},
    {NULL},
};

void parse_cmdline (int argc, char **argv, init_struct *is, int &kvm_argc, char **kvm_argv)
{
    int                 optind;
    const char          *r, *optarg;
    ...
    for (; ;)
    {
		...
		case CMDLINE_OPTION_kernel_cmd:
			 ...
			 break;
		case CMDLINE_OPTION_kvm_bootstrap:
			kvm_argv[kvm_argc] = (char *)malloc((strlen(optarg) + 1) * sizeof(char));
			strcpy(kvm_argv[kvm_argc], optarg);
			kvm_argc++;
			break;
		case CMDLINE_OPTION_kvm_software:
			kvm_argv[kvm_argc] = (char *)malloc((strlen(optarg) + 1) * sizeof(char));
			strcpy(kvm_argv[kvm_argc], optarg);
			kvm_argc++;
			break;
