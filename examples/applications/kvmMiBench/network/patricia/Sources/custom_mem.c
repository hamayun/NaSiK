...
void init_mem(custom_memory_t *pmem)
{
    ...
    for (; index < MEM_SIZE; index++)
    {
        ...
        memset(&pmem->buffer[index].item, 0x0, sizeof(pmem->buffer[index].item));
    }
...
void * alloc_mem(custom_memory_t *pmem)
{
    ...
    while(pmem->avail_count)
    {
        ...
        {
            ptr = & pmem->buffer[pmem->malloc_index].item;
...
{
    void *pitem = (void *) ptr;
    int  *pfree_flag = (int *) (pitem - sizeof(int));
...
//#define PRINT_EACH_SLOT
void print_mem_state(custom_memory_t *pmem)
{
#ifdef PRINT_EACH_SLOT
    int index = 0; 
    for (; index < MEM_SIZE; index++)
    {
        printf("[%d]: ", index);
